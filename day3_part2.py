...
class Wire(object):
    ...
    def getPath(self, wire):
        path = [[0, 0, 0]]

        for direction in wire:
            x, y, steps = path[-1]
            moveX, moveY = self.go(direction)
            if (moveX < 0):
                for i in range(x - 1, x + (moveX - 1), -1):
                    steps += 1
                    path.append([i, y, steps])

            if (moveX > 0):
                for i in range(x + 1, x + moveX + 1):
                    steps += 1
                    path.append([i, y, steps])

            if (moveY < 0):
                for i in range(y - 1, y + (moveY - 1), -1):
                    steps += 1
                    path.append([x, i, steps])

            if (moveY > 0):
                for i in range(y + 1, y + moveY + 1):
                    steps += 1
                    path.append([x, i, steps])
...
wirePathWithSteps1 = Wire().getPath(wire1)
wirePathWithSteps2 = Wire().getPath(wire2)
wirePath1 = []
wirePath2 = []
for path in wirePathWithSteps1:
    x,y,steps = path
    wirePath1.append([x,y])
    
for path in wirePathWithSteps2:
    x,y,steps = path
    wirePath2.append([x,y])

intersection = list(map(list, set(map(tuple, wirePath1)) & set(map(tuple, wirePath2))))

for x, y, steps in wirePathWithSteps1:
    check = [x, y]
    if check in intersection:
        print(check, steps)
print("\n")
for x, y, steps in wirePathWithSteps2:
    check = [x, y]
    if check in intersection:
        print(check, steps)
