...
class IntCodeComputer:
    ...
    def process(self, memory):
        ...
        while(self.processing):
            ...
            instruction = self.instructions.get(self.opcode)
            instruction()

    def processInstruction(self):
        codes = self.memory[self.currentMem]
        instructions = [int(x) for x in str(codes)]
        fill = 5 - len(instructions)
        while (len(instructions) < 5):
            instructions = ([0] * fill)[:fill] + instructions
        self.opcode = "".join([str(elem) for elem in instructions[3:5]])
        self.cMode, self.bMode, self.aMode = instructions[0:3]

    def one(self):
        a = self.argA() if self.aMode == 1 else self.memory[self.argA()]
        b = self.argB() if self.bMode == 1 else self.memory[self.argB()]
        c = self.argC()
    ...
    def three(self):
        inputInstruction = input('Input parameter:')
        argument = self.memory[self.currentMem + 1]
        self.memory[argument] = int(inputInstruction)
        self.currentMem += 2

    def four(self):
        print(self.memory[self.argA()])
        self.currentMem += 2
    ...
    def argA(self):
        return self.memory[self.currentMem + 1]

    def argB(self):
        return self.memory[self.currentMem + 2]

    def argC(self):
        return self.memory[self.currentMem + 3]


computer = IntCodeComputer()
computer.process(arr)
